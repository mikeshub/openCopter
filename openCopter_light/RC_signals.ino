void ProcessChannels(){
    ...
    if (rcCommands.values.aux1 < 1600){
      ...
      if (rollSetPoint < 0.5 && rollSetPoint > -0.5){//dead zone so it won't slowly rotate
        rollSetPoint = 0;
      }
      if (pitchSetPoint < 0.5 && pitchSetPoint > -0.5){
        pitchSetPoint = 0;
      }
      if (rateSetPointZ < 1 && rateSetPointZ > -1){
        rateSetPointZ = 0;
      }
     
    }
    else{
      MapVar(&rcCommands.values.aileron,&rateSetPointX,1000,2000,-200,200);
      MapVar(&rcCommands.values.elevator,&rateSetPointY,1000,2000,-200,200);
      MapVar(&rcCommands.values.rudder,&rateSetPointZ,1000,2000,-300,300);
       
      if (rateSetPointX < 1&& rateSetPointX > -1){//dead zone so it won't slowly rotate
        rateSetPointX = 0;
      }
      if (rateSetPointY < 1 && rateSetPointY > -1){
        rateSetPointY = 0;
      }
      if (rateSetPointZ < 1 && rateSetPointZ > -1){
        rateSetPointZ = 0;
